...
internal static class BlueprintClientUriPreservation
{
        ...
        IEnumerable<RtClientUriEntryRecord> entries)
    {
        ...
        {
            // "base" — owned by the blueprint, regenerated by the engine on every apply, no
            // preservation needed.
            ...
            // "family:*" — ephemeral, owned by BlueprintClientUriFamilyResolver, regenerated
            // from OctoIdentityServicesOptions.UriFamilies on every apply (AB#4209 Step 2b).
            // Preserving them here would resurrect family members an operator just removed
            // from env config, breaking the "family URIs follow the env config" contract.
            if (entry.Source != null
                && entry.Source.StartsWith(ClientUriSources.FamilyPrefix, StringComparison.Ordinal))
            {
                continue;
            }
