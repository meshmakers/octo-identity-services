...
/// <summary>
///     Clients with <c>RequireClientSecret = false</c> (mapped to the OpenIddict
///     public client type) may still send a <c>client_secret</c> — the pre-migration server
///     silently ignored it, while OpenIddict rejects the request with <c>invalid_client</c>
///     (ID2053). Deployed
    ...
    : IOpenIddictServerHandler<ProcessAuthenticationContext>
{
    ...
    public async ValueTask HandleAsync(ProcessAuthenticationContext context)
    {
        ...
        {
            logger.LogDebug(
                "Ignoring client_secret sent by public client '{ClientId}' (legacy-tolerant behavior)",
