...
public class IdentitiesController : ControllerBase
{
    ...
    public async Task<IActionResult> Post([Required] [FromBody] RegisterUserDto userDto)
    {
        ...
        var applicationUser = new OctoUser();
    ...
    private async Task<UserDto> CreateUserDto(OctoUser applicationUser, UserManager<OctoUser> userManager)
    {
    ...
    private async Task ApplyToUser(OctoUser octoUser, UserDto userDto)
    {
        ...
        octoUser.Roles.Clear();
        octoUser.Roles.AddRange(roleIds);
    ...
    private void LogIdentityError(string operation, IEnumerable<IdentityError> errors)
    {
        _logger.LogError("{Operation} failed with errors: '{Errors}' and codes '{Codes}'",
            ...
            string.Join(", ", errors.Select(x => x.Code)));
    }
