...
public class PasswordComplexityTooLowException : UserEmailInteractionException
{
    ...
    public static PasswordComplexityTooLowException PasswordChangeFailed(string reason, IEnumerable<IdentityError> errors)
    {
        return new PasswordComplexityTooLowException($"Password change failed: {reason}", errors);
    }
}