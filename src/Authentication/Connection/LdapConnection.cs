...
internal class LdapConnection : ILdapConnection
  {
    ...
    public List<Novell.Directory.Ldap.LdapEntry> ExecuteQuery(Action<LdapSearchParameters> configureSearchParams)
    {
    ...
    private List<Novell.Directory.Ldap.LdapEntry> SearchWithSearchQueue(
    ...
    private List<Novell.Directory.Ldap.LdapEntry> SearchWithVirtualList(
      Action<LdapSearchParameters> configureSearchParams)
    {
      var parameters = new LdapSearchParameters();
      configureSearchParams(parameters);
      ...
      var ldapEntries = new List<Novell.Directory.Ldap.LdapEntry>();
      ...
      while (pageIndex >= PageSize)
      {
        ...
        constraints.SetControls(new Novell.Directory.Ldap.LdapControl[] { ctrl, sort });
    ...
    /// <summary>
    /// Check for special case. The problem is that if the search is done with a startIndex higher then the last ldap entry, then LDAP server will always return the last entry.
    /// This can happen if the number of elements on the ldap server can be divided through the page size (eg. 300 entries with a page size of 100 will be 3 full pages).
    /// Because of the full page at the end, this function will try to get another page with an index higher than available entries on the server. The server will return
    ...
    private void CheckIfLastEntryTwice(List<Novell.Directory.Ldap.LdapEntry> ldapEntries)
    {
